...
#ifndef BELUGA_EXAMPLE_TF2_SOPHUS_HPP
#define BELUGA_EXAMPLE_TF2_SOPHUS_HPP
...
inline geometry_msgs::msg::Pose &toMsg(const Sophus::SE2<Scalar> &in,
 ...
 */
template <class Scalar>
inline geometry_msgs::msg::Pose &toMsg(const Sophus::SE3<Scalar> &in,
                                       geometry_msgs::msg::Pose &out) {
  out.position.x = in.translation().x();
  out.position.y = in.translation().y();
 ...
 */
template <class Scalar>
inline void fromMsg(const geometry_msgs::msg::Transform &msg,
 ...
 */
template <class Scalar>
inline void fromMsg(const geometry_msgs::msg::Pose &msg,
                    Sophus::SE2<Scalar> &out) {
  out.translation() = Sophus::Vector2<Scalar>{
      msg.position.x,
      msg.position.y,
...
/// Converts a Sophus (ie. Eigen) 3x3 covariance matrix to a 6x6 row-major
/// array.
/**
 * \param in A Sophus (ie. Eigen) 3x3 covariance matrix of a 2D pose (x, y,
 * yaw). \parma out A row-major array of 36 covariance values of a 3D pose.
 * \return a reference to `out`.
 */
template <template <typename, std::size_t> class Array, typename Scalar>
inline Array<Scalar, 36> &
covarianceEigenToRowMajor(const Sophus::Matrix3<Scalar> &in,
                          Array<Scalar, 36> &out) {
  out.fill(0);
  out[0] = in.coeff(0, 0);
  out[1] = in.coeff(0, 1);
 ...
 * \param out A Sophus (ie. Eigen) 3x3 covariance matrix of a 2D pose (x, y,
 * yaw). \return A Sophus (ie. Eigen) 3x3 covariance matrix of a 2D pose (x, y,
 * yaw).
 */
template <template <typename, std::size_t> class Array, typename Scalar>
inline Sophus::Matrix3<Scalar> &
covarianceRowMajorToEigen(const Array<Scalar, 36> &in,
...
// The following conversion functions need to be inside the Sophus namespace to
// make them findable by ADL when calling tf2::convert().
...
#endif // BELUGA_EXAMPLE_TF2_SOPHUS_HPP
