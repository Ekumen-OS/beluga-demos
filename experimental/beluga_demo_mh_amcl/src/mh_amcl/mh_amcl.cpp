...
namespace mh_amcl
{
        ...
        tf_listener_(tf_buffer_)
  {
    ...
    // Declare all the node's parameters necessary here
    ...
    declare_parameter<double>("init_pos_yaw", 0.0);
  }
  ...
  MH_AMCL_Node::on_activate(const rclcpp_lifecycle::State &state)
  {
    ...
    get_parameter("init_pos_x", init_pos_x_);
    get_parameter("init_pos_y", init_pos_y_);
    get_parameter("init_pos_yaw", init_pos_yaw_);
  ...
  void MH_AMCL_Node::publish_particles()
  {
    ...
    {
      utils::Color color = static_cast<utils::Color>(i % utils::NUM_COLORS);
  ...
  {
    // Don't do the processing if the first hypothesis hasn't been initialized
    if (!initial_hypothesis_initialized_)
    {
      return;
    }
      ...
      nav_msgs::msg::OccupancyGrid::ConstSharedPtr msg)
  {
    ...
    if (!initial_hypothesis_initialized_)
    {
      RCLCPP_INFO(get_logger(), "Initializing first hypothesis.");

      // Create the initial pose using the values from the parameters
      ...
      // Call init() on the primary hypothesis, passing all dependencies
      current_hypothesis_->init(init_pose, costmap_);

      // Set the flag to true to prevent this from running again
      initial_hypothesis_initialized_ = true;
    }
  }
          ...
          pose_msg)
  {
    if (pose_msg->header.frame_id == "map")
    {
      ...
      // Having a new pose shouldn't reset the whole algorithm, because that way recovering is more complicated.
      // Instead it will create a new hypothesis using that pose, assigning it as the current hypothesis with a high quality.
      // This way, if this new pose is accurate, it will remain as the current hypothesis, but if not, it will fall back to the previous hypothesis
      auto new_hypothesis = std::make_shared<ParticlesDistribution>(shared_from_this());
      new_hypothesis->on_configure(get_current_state());
      new_hypothesis->init(pose, costmap_);
      if (get_current_state().id() == lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE)
      {
        new_hypothesis->on_activate(get_current_state());
      }
      particles_population_.push_back(new_hypothesis);
      current_hypothesis_ = new_hypothesis;
      current_hypothesis_q_ = 1.0;

      // TODO: Extract this as a helper function
      // Remove the current worst hypothesis if we are over the max limit
      if (particles_population_.size() > max_hypotheses_)
      {
        RCLCPP_INFO(get_logger(), "Max hypotheses reached (%zu > %d). Pruning worst one.",
                    particles_population_.size(), max_hypotheses_);

        auto worst_hypothesis_it = particles_population_.end();
        float min_quality = std::numeric_limits<float>::max();

        // Find the non-winning hypothesis with the lowest quality score
        for (auto it = particles_population_.begin(); it != particles_population_.end(); ++it)
        {
          // Never prune the currently active hypothesis
          if (*it == current_hypothesis_)
          {
            continue;
          }

          float quality = (*it)->get_quality();
          if (quality < min_quality)
          {
            min_quality = quality;
            worst_hypothesis_it = it;
          }
        }

        // If a suitable hypothesis to remove was found, erase it
        if (worst_hypothesis_it != particles_population_.end())
        {
          RCLCPP_DEBUG(get_logger(), "Removing hypothesis with quality score: %f", min_quality);
          particles_population_.erase(worst_hypothesis_it);
        }
      }
  ...
  void MH_AMCL_Node::manage_hypotheses()
  {
    ...
    for (const auto &transform : tfs)
    {
      ...
      auto pose_tf_msg = tf2::toMsg(transform.transform);
      geometry_msgs::msg::Pose pose_msg;
      pose_msg.position.x = pose_tf_msg.translation.x;
      pose_msg.position.y = pose_tf_msg.translation.y;
      pose_msg.position.z = pose_tf_msg.translation.z;
      pose_msg.orientation = pose_tf_msg.rotation;
      bool covered = std::any_of(particles_population_.begin(), particles_population_.end(),
                                 [&](const auto &distr)
                                 {
                                   double dist_xy, theta_diff;
                                   utils::get_distances(distr->get_pose().pose.pose, pose_msg, dist_xy, theta_diff);
      ...
      if (!covered && particles_population_.size() < max_hypotheses_)
      {
        ...
        new_distr->init(transform.transform, costmap_);
    ...
    particles_population_.erase(std::remove_if(particles_population_.begin(), particles_population_.end(),
                                               [&](const auto &distr)
                                               {
                                                 ...
                                                 bool in_free = utils::get_cost(distr->get_pose().pose.pose, costmap_) == utils::FREE_SPACE;
    ...
    for (auto it1 = particles_population_.begin(); it1 != particles_population_.end(); ++it1)
    {
      ...
      {
        double dist_xy, theta_diff;
        utils::get_distances((*it1)->get_pose().pose.pose, (*it2)->get_pose().pose.pose, dist_xy, theta_diff);
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),
                        "Manage Hypotheses [" << (now() - start).seconds() << " secs]");
  }
