...
namespace mh_amcl
{
  ...
  MapMatcher::get_matches(const sensor_msgs::msg::LaserScan &scan)
  {
    ...
    std::vector<TransformWeighted> candidates[NUM_LEVEL_SCALE_COSTMAP];
    candidates[start_level] =
        get_matches(start_level, laser_poins, min_x, min_y, max_x, max_y);
    std::sort(candidates[start_level].begin(), candidates[start_level].end());
    ...
    std::sort(candidates[min_level].begin(), candidates[min_level].end());
