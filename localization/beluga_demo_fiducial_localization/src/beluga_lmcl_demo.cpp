...
namespace beluga_demo_fiducial_localization {

class LMCLFilter {
public:
  using Particle = std::tuple<Sophus::SE2d, beluga::Weight>;
  using LandmarkPositionDetectionV =
      std::vector<beluga::LandmarkPositionDetection>;

  explicit LMCLFilter(beluga::LandmarkMap &&landmark_map)
  ...
  bool update(Sophus::SE2d odometry, LandmarkPositionDetectionV detections) {
    if (particles_.empty()) {
      return false;
    }
...
private:
  ...
  static constexpr beluga::LandmarkModelParam sensor_model_params() {
    auto params = beluga::LandmarkModelParam{};
    params.sigma_bearing = 0.1;
    params.sigma_range = 0.2;
    return params;
  }
  ...
  beluga::any_policy<Sophus::SE2d, LandmarkPositionDetectionV> update_policy_;
  ...
  beluga::LandmarkSensorModel<beluga::LandmarkMap, Sophus::SE2d> sensor_model_;
...
private:
  ...
  void feature_detections_callback(
    ...
    std::vector<beluga::LandmarkPositionDetection> detections;
    ...
    for (const auto &[position, category] :
      ...
      tf2::convert(position, feature_position_in_sensor_frame);
      auto feature_position_in_base_frame =
          base_to_sensor_transform * feature_position_in_sensor_frame;
      auto &feature = detections.emplace_back();
      feature.detection_position_in_robot = feature_position_in_base_frame;
  ...
  void map_callback(const DiscreteFeatureMap::ConstSharedPtr &feature_map) {
    ...
    particle_filter_ =
        std::make_unique<LMCLFilter>(beluga::LandmarkMap{landmarks});
  ...
  std::unique_ptr<LMCLFilter> particle_filter_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(beluga_demo_fiducial_localization::LMCLNode)
